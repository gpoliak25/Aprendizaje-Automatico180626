"""
CNN Pipeline Runner + Modo Presentación
...
SLIDES = [
    ...
    {"title": "🎛 Demo: Calibración de Umbral",                "file": None},
    {"title": "🩻 Demo: Predicción en Vivo",                   "file": None},
]
N_TOTAL = len(SLIDES)  # 15
...
<style>
  ...
  *,p,span,label,div { color:#c9d1d9; }
  ...
  .stButton>button:hover {
    background:#388bfd !important; transform:translateY(-2px);
  ...
  .btn-reset .stButton>button:hover { background:#30363d !important;
    box-shadow:none !important; transform:none !important; }

  /* Botones de navegación: más compactos */
  .nav-btn .stButton>button {
    min-height:44px !important; padding:.5rem 1rem !important; font-size:.9rem !important;
  }
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODO PRESENTACIÓN — funciones de renderizado
# ═══════════════════════════════════════════════════════════════════════════════

def _demo_umbral():
    """Paso interactivo 14: calibración de umbral en vivo."""
    import plotly.graph_objects as go

    st.markdown("## 🎛 Calibración de Umbral en Vivo")
    st.markdown("""
    <p style="color:#8b949e;margin-bottom:1rem;">
      Mové el umbral y observá cómo cambia el trade-off entre
      <strong>Recall</strong> (sensibilidad) y <strong>Precisión</strong>.
      El objetivo clínico es minimizar falsos negativos (↑ Recall).
    </p>""", unsafe_allow_html=True)

    umbral = st.slider(
        "Umbral de decisión",
        min_value=0.10, max_value=0.90, value=0.30, step=0.01,
        key="umbral_pres_slider",
        help="Umbral sobre la probabilidad predicha para clasificar como 'patológico'.",
    )

    # TODO: reemplazar con datos reales del test.
    # Cargar CSV con columnas [prob_patologica, y_true] y recalcular en tiempo real:
    #   df = pd.read_csv("assets/probs_test.csv")
    #   preds = (df["prob_patologica"] >= umbral).astype(int)
    #   recall = recall_score(df["y_true"], preds)
    #   prec   = precision_score(df["y_true"], preds, zero_division=0)
    # Por ahora: interpolación lineal entre los 2 puntos conocidos del TP.
    # Punto A: umbral=0.50 → Recall=0.767, Precision=0.793
    # Punto B: umbral=0.30 → Recall=0.867, Precision=0.650

    def _interp(x, x0, y0, x1, y1, lo, hi):
        t = (x - x0) / (x1 - x0)
        return max(lo, min(hi, y0 + t * (y1 - y0)))

    if umbral < 0.30:
        recall = _interp(umbral, 0.10, 0.97,  0.30, 0.867, 0.60, 0.99)
        prec   = _interp(umbral, 0.10, 0.45,  0.30, 0.650, 0.40, 0.70)
    elif umbral <= 0.50:
        recall = _interp(umbral, 0.30, 0.867, 0.50, 0.767, 0.60, 0.99)
        prec   = _interp(umbral, 0.30, 0.650, 0.50, 0.793, 0.60, 0.85)
    else:
        recall = _interp(umbral, 0.50, 0.767, 0.90, 0.10,  0.05, 0.80)
        prec   = _interp(umbral, 0.50, 0.793, 0.90, 0.98,  0.75, 1.00)

    f1 = 2 * prec * recall / (prec + recall) if (prec + recall) > 0 else 0.0

    c1, c2, c3 = st.columns(3)
    c1.metric("Recall (Sensibilidad)", f"{recall:.3f}",
              delta=f"{recall - 0.767:+.3f} vs umbral 0.50")
    c2.metric("Precision", f"{prec:.3f}",
              delta=f"{prec - 0.793:+.3f} vs umbral 0.50")
    c3.metric("F1-Score", f"{f1:.3f}")

    if umbral <= 0.35:
        color, msg = "#3fb950", "✅ Modo triage clínico: prioriza capturar patologías (↑ Recall)"
    elif umbral >= 0.65:
        color, msg = "#388bfd", "📌 Alta precisión: menos falsos positivos, más falsos negativos"
    else:
        color, msg = "#d29922", "⚖ Balance intermedio entre Recall y Precision"

    st.markdown(f"""
    <div style="background:#161b22;border-left:4px solid {color};border-radius:8px;
                padding:14px 18px;margin:12px 0;color:#c9d1d9;font-size:.9rem;">
      {msg}
    </div>""", unsafe_allow_html=True)

    # Curva recall/precision en función del umbral
    t_vals = [i / 100 for i in range(10, 91)]
    r_vals, p_vals = [], []
    for t in t_vals:
        if t < 0.30:
            r_vals.append(_interp(t, 0.10, 0.97,  0.30, 0.867, 0.60, 0.99))
            p_vals.append(_interp(t, 0.10, 0.45,  0.30, 0.650, 0.40, 0.70))
        elif t <= 0.50:
            r_vals.append(_interp(t, 0.30, 0.867, 0.50, 0.767, 0.60, 0.99))
            p_vals.append(_interp(t, 0.30, 0.650, 0.50, 0.793, 0.60, 0.85))
        else:
            r_vals.append(_interp(t, 0.50, 0.767, 0.90, 0.10,  0.05, 0.80))
            p_vals.append(_interp(t, 0.50, 0.793, 0.90, 0.98,  0.75, 1.00))

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=t_vals, y=r_vals, name="Recall",
                             mode="lines", line=dict(color="#d29922", width=2)))
    fig.add_trace(go.Scatter(x=t_vals, y=p_vals, name="Precision",
                             mode="lines", line=dict(color="#388bfd", width=2)))
    fig.add_vline(x=umbral, line_dash="dash", line_color="#484f58", line_width=1,
                  annotation_text=f"  {umbral:.2f}", annotation_font_color="#8b949e")
    fig.add_trace(go.Scatter(
        x=[umbral, umbral], y=[recall, prec], mode="markers",
        marker=dict(size=10, color=["#d29922", "#388bfd"]), showlegend=False,
    ))
    fig.update_layout(
        paper_bgcolor="#0d1117", plot_bgcolor="#0d1117", font_color="#c9d1d9",
        height=220, margin=dict(t=10, b=40, l=40, r=10),
        xaxis=dict(title="Umbral", gridcolor="#21262d", zeroline=False, range=[0.10, 0.90]),
        yaxis=dict(title="Valor",  gridcolor="#21262d", zeroline=False, range=[0, 1.05]),
        legend=dict(bgcolor="#0d1117", bordercolor="#21262d", borderwidth=1, font_size=11),
    )
    st.plotly_chart(fig, use_container_width=True, key="umbral_chart_pres")
    st.caption("📌 Curva interpolada sobre los 2 puntos conocidos (umbral 0.30 y 0.50). "
               "Conectar assets/probs_test.csv para curva real del conjunto de test.")


def _demo_prediccion():
    """Paso interactivo 15: predicción en vivo con ONNX."""
    st.markdown("## 🩻 Predicción en Vivo")

    # Auto-detección del modelo ONNX
    default_model_dirs = [
    ...
    if found_models:
        ...
        model_name = found_models[0].name
        try:
        ...
        st.warning("⚠️ No se encontró modelo ONNX. "
                   "Copiá `transfer_mobilenetv2.onnx` en la carpeta `modelos/`.")
    ...
    with col_upload:
        ...
        if uploaded:
            ...
            predict_btn = st.button("▶  Predecir", use_container_width=True, key="demo_pres_predict")
        else:
            st.markdown("""
    ...
    with col_result:
        ...
        if uploaded and predict_btn:
            if model is None:
                result_ph.error("❌ No hay modelo cargado.")
            else:
                        ...
                        <div style="background:#161b22;border:1px solid #21262d;border-radius:12px;
                          ...
                          <div style="margin-bottom:10px">
                            ...
                            <div style="background:#21262d;border-radius:4px;height:8px;">
                              <div style="background:#3fb950;border-radius:4px;height:8px;
                                          width:{p_ok:.1f}%"></div>
                            </div>
                          </div>
                          <div>
                            ...
                            <div style="background:#21262d;border-radius:4px;height:8px;">
                              <div style="background:#f85149;border-radius:4px;height:8px;
                                          width:{p_pat:.1f}%"></div>
                            </div>
                          </div>
...
def _render_presentacion():
    """Modo Presentación: deck guiado de diapositivas con demo final."""
    paso = st.session_state.paso

    # CSS: maximizar espacio, ocultar chrome de Streamlit
    st.markdown("""<style>
    ...
    # Barra de contexto minimalista
    st.markdown(
        f'<div style="color:#484f58;font-size:.72rem;margin-bottom:6px;">'
        f'🩻 CNN Pipeline · Radiografías Veterinarias &nbsp;·&nbsp;'
        f'<span style="color:#8b949e">{SLIDES[paso]["title"]}</span>'
        f'</div>',
        unsafe_allow_html=True,
    )
    ...
    if paso < 13:
        ...
        if img_path.exists():
            ...
            <div style="background:#161b22;border:2px dashed #30363d;border-radius:14px;
              ...
              <div style="color:#484f58;font-size:.85rem;margin-bottom:12px">
                Diapositiva {paso + 1} / 13</div>
              <div style="color:#e6edf3;font-size:2rem;font-weight:800;margin-bottom:16px">
                {slide['title']}</div>
              <div style="color:#484f58;font-size:.8rem">
                PNG no encontrado — colocá
                <code style="color:#58a6ff">assets/slides/{slide['file']}</code>
              </div>
            </div>""", unsafe_allow_html=True)

        # Botón "Iniciar demo en vivo" al pie de la última diapositiva estática
        if paso == 12:
            st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
            _, col_dem, _ = st.columns([3, 2, 3])
            with col_dem:
                if st.button("▶  Iniciar demo en vivo", use_container_width=True,
                             key="btn_start_demo"):
                    st.session_state.paso = 13
                    st.rerun()
        ...
        _demo_prediccion()
        st.divider()
        col_back, col_switch = st.columns(2)
        with col_back:
            if st.button("↩ Volver al inicio de la presentación",
                         use_container_width=True, key="btn_volver_inicio"):
                st.session_state.paso = 0
                st.rerun()
        with col_switch:
            if st.button("🧪 Ir al modo Demo / Exploración",
                         use_container_width=True, key="btn_ir_demo"):
    ...
    with col_prev:
        ...
        if st.button("◀ Anterior", disabled=(paso == 0),
                     use_container_width=True, key="btn_prev", type="secondary"):
            st.session_state.paso -= 1
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    with col_info:
        st.markdown(
            f'<div style="text-align:center;color:#8b949e;font-size:.8rem;padding:.5rem 0;">'
    ...
    with col_next:
        st.markdown('<div class="nav-btn">', unsafe_allow_html=True)
        if paso < N_TOTAL - 1:
            if st.button("Siguiente ▶", use_container_width=True, key="btn_next"):
                st.session_state.paso += 1
                st.rerun()
        else:
            if st.button("↩ Inicio", use_container_width=True, key="btn_restart_nav"):
                st.session_state.paso = 0
                st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODO DEMO — todas las pestañas de herramientas
# ═══════════════════════════════════════════════════════════════════════════════

def _render_demo():
    """Modo Demo / Exploración: pestañas con Pipeline Runner, Notebooks, Predicción y Presentación."""
    import plotly.graph_objects as go
    ...
    with tab_pipeline:
        ...
        if not demo_mode:
            base_dir = st.text_input(
                "base_dir", label_visibility="collapsed",
        ...
        if run_targets is not None:
            ...
            if all_ok:
                ...
                failed = [PIPELINE[i]["title"] for i in run_targets
                          if st.session_state.statuses[i] == "error"]
    ...
    # ── TAB: Predicción en vivo ───────────────────────────────────────────────
    with tab_pred:
        st.markdown("""
        <p style="color:#8b949e;margin-bottom:1.2rem;">
          Cargá un modelo entrenado (<code>.onnx</code>) y subí una radiografía
        ...
        with col_btn:
            ...
            st.button("⬇ Cargar", use_container_width=True, key="demo_load_btn")
        ...
        if model_path and Path(model_path).is_file():
            ...
            st.warning("⚠️  Archivo no encontrado. Verificá la ruta o descargá el modelo.")
                        ...
                        try:
                            ...
                            <div style="background:#161b22;border:1px solid #21262d;border-radius:12px;
                              ...
                              <div style="margin-top:20px;padding-top:16px;
                                          border-top:1px solid #21262d;color:#484f58;font-size:.75rem;">
    ...
    with tab_pres:
            ...
            <div style="background:#0d1117;border:1px solid #21262d;border-radius:8px;padding:18px">
              ...
              <div style="display:flex;flex-direction:column;gap:8px">
                <div style="background:#21262d;border-radius:6px;padding:10px 14px;
                            color:#c9d1d9;font-size:.84rem">👤  Ingreso del Paciente y Toma de Placa</div>
                <div style="text-align:center;color:#484f58">↓</div>
                <div style="background:#21262d;border-radius:6px;padding:10px 14px;
                            color:#c9d1d9;font-size:.84rem">🕐  Espera en Cola de Lectura</div>
                <div style="text-align:center;color:#484f58">↓</div>
                <div style="background:#21262d;border-radius:6px;padding:10px 14px;
                            color:#c9d1d9;font-size:.84rem">👁  Análisis Manual por Especialista</div>
              </div>
            </div>""", unsafe_allow_html=True)
        with col_ai:
            st.markdown("""
            <div style="background:#0d1117;border:1px solid #388bfd;border-radius:8px;padding:18px">
              ...
              <div style="display:flex;flex-direction:column;gap:8px">
                ...
                <div style="text-align:center;color:#484f58">↓</div>
                <div style="background:#21262d;border-radius:6px;padding:10px 14px;
                            color:#c9d1d9;font-size:.84rem">👨‍⚕️  Análisis Enfocado por Especialista</div>
              </div>
            </div>""", unsafe_allow_html=True)
        with col_pts:
        ...
        with col_pie:
            fig_pie = go.Figure(go.Pie(
                labels=["Normal (ok)", "Patológica"], values=[245, 196],
                hole=0.55, marker_colors=["#58a6ff", "#d29922"],
        ...
        <div style="display:grid;grid-template-columns:auto 1fr 1fr;gap:6px;max-width:640px;margin:0 auto 16px">
          ...
          <div style="background:#2d0f0f;border:1px solid #f85149;border-radius:6px;
          ...
          <div></div>
          <div style="background:#2d1f0f;border:1px solid #d29922;border-radius:6px;
                      padding:18px;text-align:center">
        ...
        def arch_layer(name, extra=""):
            return (f'<div style="background:#0d1f38;border:1px solid #388bfd;border-radius:5px;'
        ...
        # ── SLIDE 11-12 · Anatomía del Error ─────────────────────────────────
        ...
        for col, num, title, color, body in [
            (col_f1, "1", "Validación Cruzada (K-fold)", "#388bfd",
             "Robustecer las estimaciones métricas mitigando la varianza del conjunto de test reducido."),
        ...
        # Botón para saltar al modo presentación
        st.divider()
        _, col_btn_pres, _ = st.columns([3, 2, 3])
        with col_btn_pres:
            if st.button("🎤 Abrir Modo Presentación", use_container_width=True,
                         key="demo_open_pres"):
                st.session_state.modo = "presentacion"
                st.session_state.paso = 0
                st.rerun()
...
with st.sidebar:
    ...
    modo_label = st.radio(
        "Modo",
        ["🎤 Presentación", "🧪 Demo / Exploración"],
        index=0 if st.session_state.modo == "presentacion" else 1,
        label_visibility="collapsed",
        key="sidebar_modo_radio",
    )
    nuevo_modo = "presentacion" if modo_label.startswith("🎤") else "demo"
    ...
    if st.session_state.modo == "presentacion":
        st.divider()
        st.markdown("##### 📑 Ir a diapositiva")
        opciones_sidebar = [f"{i+1}. {s['title']}" for i, s in enumerate(SLIDES)]
        paso_actual = min(st.session_state.paso, N_TOTAL - 1)
        sel_sidebar = st.selectbox(
            "",
            opciones_sidebar,
            index=paso_actual,
            label_visibility="collapsed",
            key="sidebar_jump_sel",
        )
        nuevo_paso_sidebar = opciones_sidebar.index(sel_sidebar)
        if nuevo_paso_sidebar != st.session_state.paso:
            st.session_state.paso = nuevo_paso_sidebar
            st.rerun()

        st.divider()
        if st.button("↩ Reiniciar presentación", use_container_width=True,
                     key="sidebar_restart"):
            st.session_state.paso = 0
            st.rerun()

    st.markdown(
        '<div style="margin-top:2rem;color:#484f58;font-size:.72rem;line-height:1.6">'
        'TP Final · Aprendizaje Automático<br>'
        'Universidad CAECE · Junio 2026<br>'
        'Lic. Lorena López · Lic. Gisela Poliak'
        '</div>',
        unsafe_allow_html=True,
    )
